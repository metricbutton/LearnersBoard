...
{
    public partial class approvecertificates : System.Web.UI.Page
    {
        ...
        protected void LinkButton4_Click(object sender, EventArgs e) //Go
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * from certificate_master_tbl where document_id='" + TextBox1.Text.Trim() + "';", con);
                ...
                if (dt.Rows.Count >= 1)
                {
                    ...
                    TextBox8.Text = dt.Rows[0][3].ToString(); //Date
                    TextBox3.Text = dt.Rows[0][4].ToString(); //Type
                    TextBox4.Text = dt.Rows[0][2].ToString(); //Full Name
                    TextBox6.Text = dt.Rows[0][7].ToString(); //Event Name
                    HiddenField1.Value = dt.Rows[0][8].ToString(); //Document Path
                }
                else
                {
        ...
        protected void LinkButton1_Click(object sender, EventArgs e) //Approve
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
                ...
                SqlCommand cmd = new SqlCommand("UPDATE certificate_master_tbl SET status=@status WHERE document_id='" + TextBox1.Text.Trim() + "'", con);
        ...
        protected void LinkButton2_Click(object sender, EventArgs e) //Disapprove
        {
            try
            {
                SqlConnection con = new SqlConnection(strcon);
        ...
        void updateGridView()
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT document_id,learner_username,learner_fullname,date,type,event_name from certificate_master_tbl where coordinator_id='" + Session["coordinator_id"].ToString() + "' and status='Pending'; ", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
